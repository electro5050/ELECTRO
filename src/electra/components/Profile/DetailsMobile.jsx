...
const Details = () => {
  ...
  const handleFileSelected = async (e) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      try {
        const response = await axios.get('http://192.168.29.85:3000/generate-presigned-url', {
        ...
        if (response.data.preSignedUrl) {
          ...
          if (uploadResult.ok) {
            ...
            const updateResponse = await axios.post('http://192.168.29.85:3000/update-profile-picture', {
              userId: user[0].id, 
